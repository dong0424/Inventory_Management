...
class GoodsDialog(QDialog):
    def __init__(self):
        ...
        # 버튼 클릭 이벤트 처리
        self.pushButton_goods1.clicked.connect(self.onSearchClick)
    ...
    def onSearchClick(self):
        ...
        if search_string:  # 검색 문자열이 있을 때만 검색을 수행
            print('Search good')
            result = self.myDB.goods_search(search_string)

            if result:
                if result['codenumber'] == search_string:
                    self.lineEdit_1.setText(result['codenumber'])
                    self.lineEdit_2.setText(result['goodname'])
                    self.lineEdit_3.setText(result['Initial_inventory'])
                    self.lineEdit_4.setText(result['price'])
                    self.searched = True
                else:
                    self.clearLineEdit()
